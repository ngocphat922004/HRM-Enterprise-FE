...
export class NhanVienService {
    ...
    private readonly apiUrl =
        `${environment.apiBaseUrl}${API_ENDPOINTS.nhanVien}`;

    private readonly meUrl =
        `${environment.apiBaseUrl}${API_ENDPOINTS.nhanVienMe}`;

    private readonly importPreviewUrl =
        `${environment.apiBaseUrl}${API_ENDPOINTS.nhanVienImport.preview}`;

    private readonly importUrl =
        `${environment.apiBaseUrl}${API_ENDPOINTS.nhanVienImport.import}`;

    getAll(): Observable<NhanVienChiTiet[]> {
        return this.http
            .get<
                ApiResponse<NhanVienChiTiet[]> |
                NhanVienChiTiet[]
            >(this.apiUrl)
            .pipe(
                map((response) =>
                    this.unwrapList(response),
                ),
            );
    }
        ...
        return this.http
            ...
            .pipe(
                map((response) => {
                    ...
                    if (!employee) {
                        throw new Error(
                            'Không nhận được thông tin nhân viên hiện tại.',
                        );
                    }
        ...
        return this.http
            ...
            .pipe(
                map((response) => {
                    ...
                    if (!employee) {
                        throw new Error(
                            'Không nhận được dữ liệu nhân viên vừa tạo.',
                        );
                    }
    ...
    previewImport(
        file: File,
    ): Observable<NhanVienImportPreviewRow[]> {
        ...
        return this.http
            .post<
                ApiResponse<NhanVienImportPreviewRow[]> |
    ...
    private assertSuccess(
        ...
        if (
            ...
            throw new Error(
                ...
                'Thao tác nhân viên không thành công.',
            );
        }
    }
