...
export type CreatePhongBanRequest =
    Omit<PhongBan, 'maPB'>;

export type UpdatePhongBanRequest =
    CreatePhongBanRequest;

type PhongBanApiItem =
    Omit<PhongBan, 'trangThai'> & {
        trangThai?:
        PhongBan['trangThai'] |
        null;
    };
...
export class PhongBanService {
    ...
    private readonly apiUrl =
        `${environment.apiBaseUrl}${API_ENDPOINTS.phongBan}`;

    getAll(): Observable<PhongBan[]> {
        return this.http
            .get<
                ApiResponse<PhongBanApiItem[]> |
                PhongBanApiItem[]
            >(this.apiUrl)
            .pipe(
                map((response) => {
        ...
        return this.http
            ...
            .pipe(
                map((response) => {
                    ...
                    if (!department) {
                        throw new Error(
                            'Không nhận được dữ liệu phòng ban.',
                        );
                    }
        ...
        return this.http
            ...
            .pipe(
                map((response) => {
                    ...
                    if (!department) {
                        throw new Error(
                            'Không nhận được dữ liệu phòng ban vừa tạo.',
                        );
                    }
    ...
    update(
        ...
        return this.http
            .put<
                ApiResponse<PhongBanApiItem | null> |
                PhongBanApiItem |
                null
            >(
                `${environment.apiBaseUrl}${API_ENDPOINTS.phongBanById(maPB)}`,
    ...
    private assertSuccess(
        ...
        if (
            ...
            throw new Error(
                ...
                'Thao tác phòng ban không thành công.',
            );
        }
    }
